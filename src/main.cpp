                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
    ...
    // Diminutivecoin: in v2 transactions use GetAdjustedTime() as nTimeTx
...

    {
        ...
        // Diminutivecoin: Minimum fee check
...
////////////////////////////////////////////////////////////
///
/// DiminutiveCoin dual value POW reward:
///

CAmount GetProofOfWorkSubsidy()
{
    // Premine for a swap from old chain
    if (chainActive.Height() == 1)
        return 30000 * COIN;
    
    // Predefined high and low rewards for each halving interval (POW)
    static const CAmount powHighRewards[] = {
        static_cast<CAmount>(0.01 * COIN),        // High reward, initial interval
        static_cast<CAmount>(0.005 * COIN),       // High reward, first halving
        static_cast<CAmount>(0.0025 * COIN),      // High reward, second halving
        static_cast<CAmount>(0.00125 * COIN),     // High reward, third halving
        static_cast<CAmount>(0.000625 * COIN),    // High reward, fourth halving
        static_cast<CAmount>(0.0003125 * COIN),   // High reward, fifth halving
        static_cast<CAmount>(0.00015625 * COIN),  // High reward, sixth halving
        static_cast<CAmount>(0.00078125 * COIN)   // High reward, seventh halving
    };
    static const CAmount powLowRewards[] = {
        static_cast<CAmount>(0.0025 * COIN),      // Low reward, initial interval
        static_cast<CAmount>(0.00125 * COIN),     // Low reward, first halving
        static_cast<CAmount>(0.000625 * COIN),    // Low reward, second halving
        static_cast<CAmount>(0.0003125 * COIN),   // Low reward, third halving
        static_cast<CAmount>(0.00015625 * COIN),  // Low reward, fourth halving
        static_cast<CAmount>(0.00007812 * COIN),  // Low reward, fifth halving
        static_cast<CAmount>(0.00003906 * COIN),  // Low reward, sixth halving
        static_cast<CAmount>(0.00001953 * COIN)   // Low reward, seventh halving
    };

    // Determine the current halving level
    int halvingIndex = std::min(chainActive.Height() / 500000, (int)(sizeof(powHighRewards) / sizeof(powHighRewards[0]) - 1));

    // Alternate between high and low rewards based on block parity
    if (chainActive.Height() % 2 == 0)
        return powHighRewards[halvingIndex]; // High reward for even blocks
    else
        return powLowRewards[halvingIndex]; // Low reward for odd blocks
}

////////////////////////////////////////////////////////////
///
/// DiminutiveCoin dual value POS reward:
///

CAmount GetProofOfStakeSubsidy()
{
    // Predefined high and low rewards for each halving interval (POS)
    static const CAmount posHighRewards[] = {
        static_cast<CAmount>(0.0025 * COIN),       // High reward, initial interval
        static_cast<CAmount>(0.00125 * COIN),      // High reward, first halving
        static_cast<CAmount>(0.000625 * COIN),     // High reward, second halving
        static_cast<CAmount>(0.0003125 * COIN),    // High reward, third halving
        static_cast<CAmount>(0.00015625 * COIN),   // High reward, fourth halving
        static_cast<CAmount>(0.00007812 * COIN),   // High reward, fifth halving
        static_cast<CAmount>(0.00003906 * COIN),   // High reward, sixth halving
        static_cast<CAmount>(0.00001953 * COIN)    // High reward, seventh halving
    };
    static const CAmount posLowRewards[] = {
        static_cast<CAmount>(0.0000025 * COIN),    // Low reward, initial interval
        static_cast<CAmount>(0.00000125 * COIN),   // Low reward, first halving
        static_cast<CAmount>(0.00000062 * COIN),   // Low reward, second halving
        static_cast<CAmount>(0.00000031 * COIN),   // Low reward, third halving
        static_cast<CAmount>(0.00000015 * COIN)    // Low reward, last halving.
    };

    // Determine the current halving level
    int halvingIndex = std::min(chainActive.Height() / 1000000, (int)(sizeof(posHighRewards) / sizeof(posHighRewards[0]) - 1));
    // Alternate between high and low rewards based on block parity
    if (chainActive.Height() % 2 == 0)
        return posHighRewards[halvingIndex]; // High reward for even blocks
    else
        return posLowRewards[halvingIndex]; // Low reward for odd blocks
}
...
void ThreadScriptCheck() {
    RenameThread("diminutivecoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // DiminutiveCoin also requires DER encoding of pubkeys
    flags |= SCRIPT_VERIFY_DERKEY;
    // DiminutiveCoin also requires low S in sigs
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    if (block.nVersion < 1 && consensusParams.IsProtocolV2(block.GetBlockTime()))
